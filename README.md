...
The application supports selectable TTS (Text-to-Speech) models. The primary/default path is Google Cloud TTS Neural2. Configure defaults in `voice_config.py`:

```python
# Default TTS model (Google Cloud TTS Neural2)
...
| Model | Description |
|-------|-------------|
| `google-cloud-neural2` | Google Cloud TTS with `th-TH-Neural2-C` (default) |
...
Users can also select the TTS model, Cloud voice, and Gemini style prompt from the **TTS Settings** panel in the Voice Mode pre-brief page before starting a case.
